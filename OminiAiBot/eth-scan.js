...
// ⚠️ Ethereum mainnet NodeReal endpoint (apni ETH API key yahan daalo)
const NODEREAL_API = "https://eth-mainnet.nodereal.io/v1/a64423827429445db203a57251663414";

// Ethereum par USDT aur USDC ke contract addresses (lowercase)
...
function parseTransfer(t) {
  ...
  if (category === "external") {
    ...
    asset = "USDT";
    decimals = 6; // ✅ Ethereum par USDT = 6 decimals
  } else if (contractAddr === USDC_CONTRACT) {
    asset = "USDC";
    decimals = 6; // ✅ Ethereum par USDC = 6 decimals
  } else {
    asset = assetFromApi || "TOKEN";
    // baaki tokens ke liye NodeReal ka decimal field use karo
    if (t.decimal) {
...
async function main() {
  ...
  const askAddress = () => {
    return new Promise((resolve) => {
      rl.question("\n🔍 Enter ETH wallet address: ", (answer) => {
        resolve(answer.trim());
      });
    });
  };
